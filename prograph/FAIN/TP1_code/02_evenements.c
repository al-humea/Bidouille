...
typedef struct
{ int nb_sommets; point sommets[100];} Polygone;
...
void P_nouveauSommet(Polygone *p, int x, int y){
	p->sommets[p->nb_sommets].x = x;
	p->sommets[p->nb_sommets].y = y;
	p->nb_sommets++;
}
int pfill;
void P_draw(Polygone *p){
	glBegin(pfill==0?GL_LINE_STRIP:GL_POLYGON);
	for (int i = 0; i < p->nb_sommets; i++){
		glVertex2i(p->sommets[i].x, p->sommets[i].y);
	}
	glVertex2i(p->sommets[0].x, p->sommets[0].y);
	glEnd();
}
